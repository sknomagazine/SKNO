export const TYPES = {
  ...
  ezine: { file: "data/ezines.json", label: "eZine" },
...
const dateValid = (value) => /^\d{4}-\d{2}-\d{2}$/.test(value) && !Number.isNaN(Date.parse(`${value}T00:00:00Z`));
...
export function validatePublication(type, input) {
  ...
  if (type === "ezine") { item.pdf = validMediaPath(input.pdf, "ezine") || url(input.pdf); if (!item.pdf) throw Object.assign(new Error("Envie um PDF ou informe uma URL válida."), { status: 400, code: "INVALID_MEDIA" }); if (input.autor) item.autor = clean(input.autor, 160); }
...
export function validMediaPath(value, type) {
  ...
  return value.startsWith(`uploads/${type === "ezine" ? "ezines" : type === "arte" ? "artes" : type === "musica" ? "musicas" : "filmes"}/`) ? value : "";
}

export function validateUpload({ type, slug, kind, name, mime, size }) {
  if (!TYPES[type] || !["ezine", "arte", "musica", "filme"].includes(type)) throw Object.assign(new Error("Tipo inválido."), { status: 400, code: "INVALID_TYPE" });
  if (slugify(slug) !== slug) throw Object.assign(new Error("Slug inválido."), { status: 400, code: "INVALID_SLUG" });
  ...
  const folder = { ezine: "ezines", arte: "artes", musica: "musicas", filme: "filmes" }[type];
...
export function matchesMagicBytes(kind, mime, bytes) {
  ...
  if (kind === "image" && mime === "image/png") return bytes.slice(0, 8).every((v, i) => v === [137, 80, 78, 71, 13, 10, 26, 10][i]);
