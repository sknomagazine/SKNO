...
test("token GitHub ausente é detectado", async () => { const response = await publications(context("GET")); assert.equal(response.status, 503); assert.equal((await response.json()).code, "MISSING_GITHUB_TOKEN"); });
test("GitHub indisponível não expõe resposta autenticada", async t => { const original = globalThis.fetch; globalThis.fetch = async () => new Response("secret detail", { status: 500 }); t.after(() => globalThis.fetch = original); const response = await publications(context("GET", null, { GITHUB_TOKEN: "fake", GITHUB_OWNER: "x", GITHUB_REPO: "y" })); assert.equal(response.status, 502); assert.doesNotMatch(await response.text(), /secret detail|fake/); });
